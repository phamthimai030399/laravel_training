...
class ClientAuthController extends Controller
{
    ...
    public function postLogin(LoginRequest $request)
    {
        ...
        if ($result['type'] && $result['active']) {
            return redirect(route('client.home'))->with('message', Message::success('Đăng nhập thành công'));
    ...
    public function postVerifyChangePassword($token, VerifyChangePasswordRequest $request)
    {
        ...
        if ($result['status']) {
            return redirect(route('client.view_login'))->with('message', Message::success('Thay đổi mật khẩu thành công'));
        } elseif(!empty($result['type']) && $result['type'] == 'expried') {
            return redirect(route('client.forgot_password'))
    ...
    public function postChangePassword(ChangePasswordRequest $request)
    {
        ...
        if ($result) {
            return redirect(route('client.home'))->with('message', Message::success('Đổi mật khẩu thành công'));
        } else {
